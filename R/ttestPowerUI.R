...
ttestPowerUI <- function(id) {
  ...
  tagList(
    ...
    column(12,style = " background-color: white; border-radius: 5px; ",
           ...
           column(width=12,
                  "You've already decided that you're conducting this analysis for a 2-Sample T-Test, given that you picked that tab above. Now, decide whether you want to use an unstandardized effect size (specify difference in means and standard deviation) or a standardized effect size (use Glass's/Cohen's d). Click the corresponding tab below. Click the buttons next to 'Learn:' in the top right of this white box for more information.",
                  br(),
                  br(),
                 "If you like, you can upload an analysis file to set the options for this tool. To see an example analysis file for this tool, click here:",
                 br(),
                 br(),
                 tags$a(href="power2SampTTestIn.xlsx",download="power2SampTTestIn.xlsx",
                        "Example Analysis File for 2-Sample T-Test Power/Sample Size/Effect Size Analysis",target="_blank"),
                 br(),
                 br(),
                 "You can also upload the Analysis file saved after using this tool previously. This is the tab named 'Analysis' in the reproducible file you downladed. The fact that you can upload that file and get the exact same results is what makes this reproducible research (see last section). Select an analysis file to upload here:",
                 
                 br(),
                 br(),
                 
                  fileInput(ns("tTestPowerData"),"Enter data file location",accept=".xlsx"),
                  br(),
                  br(),
                  br()
           ),
  ...
  moduleServer(
    ...
    function(input, output, session) {
      ...
      content=function(file){
        ...
        header3<-matrix(c("2-Sample T-Test Power/Sample Size/Effect Size","Core R Packages and Functions: power.t.test{stats}"),ncol=1)
        ...
        writeData(wb,sheet=sheet,x="No plot output available for this tool.", startCol=2, startRow=2, rowNames=FALSE,colNames=FALSE)
