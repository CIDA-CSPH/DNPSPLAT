...
column(width=9, style="background-color: white",
       br(),
       br(),
       h1("Research Process Flow Home"),
       br(),
       br(),
       "Note for myself (and possibly for users): The site consists of the home/landing page, which is quite simple and specific to the audience, the learning modules, which is an entire top-level section and specific to the audience, and the analysis tools (and demonstration tools, like the CLT tool). These tools can be presented/used in two ways: 1) in the Explore section, and 2) in the Research Process Flow section. Using the explore section is like looking at the steps in the research process flow chart from the outside looking in--the type of learning where one learns the outline, then fills in the details. This is the way that the example data will usually be accessed, and is unlikely to involve people using their own data. The research process flow section is where users will experience the research process by flowing through it, mainly one step after another. This involves the type of learning where one starts at the beginning of the book and reads straight through in order. This is unlikely to involve users using the example data but is more likely to involve users uploading their own data. The explore section is more likely to be used in a short course and the flow section is more likely to be used by students like the DNP students, by investigators doing their own analysis, or even by a statistician. The explore layout has the tools in tabs such that you can see multiple at once, like looking at the flow chart on the page all at once/zoomed out so you can see the relative positions/relationships among the tools, while the layout of the tools in the flow section isn't really even a layout because you just start from one home page for the flow section, then see the tools in pop out/modal windows that you can scroll through in order like you can with the videos in the learning modules, though there should be an overview that you can see with a zoom out type of function.",
       br(),
       br(),
       ...
       button) to learn more. Try it here for an overview of the use of analysis files with this tool:",
       ...
       tabsetPanel(type = "tabs",
                   ...
                   tabPanel(h4("Uploaded Analysis Files"),
