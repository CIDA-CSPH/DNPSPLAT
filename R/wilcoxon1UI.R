...
wilcoxon1UI <- function(id) {
  ...
  tagList(
    ...
    column(12,style = " background-color: white; border-radius: 5px; ",
           ...
           column(12,
                  br(),
           "For a Wilcoxon Rank-Sum/Mann-Whitney U test, the null hypothesis is that the distributions of the numeric outcome in the two groups differ by a location shift of \u03B8 and the alternative is that they differ by some other location shift.",
           br(),
           br(),
           br()
           ),
           ...
           column(width=12,
                  br(),
           HTML(paste0(
             "Enter your null hypothesis H",tags$sub("0")," for the shift in population distributions \u03B8:"
           )),
           br(),
           br(),
                              ...
                              tabPanel("Upload Data",value=3,
                                       ...
                                       fluidPage(
                                         column(12,style = " background-color: white; border-radius: 5px; ",
                                                ...
                                                br(),
                                                br(),
                                                tags$a(href="wilcoxonTest1Data.xlsx",download="wilcoxonTest1Data.xlsx",
                                                       "Example Analysis File for Wilcoxon Rank-Sum Test",target="_blank"),
                                                br(),
                                                br(),
                                                ...
                                                br(),
                                                br(),
                                                fileInput(ns("wilcoxonTest1Data"),"Enter data file location",accept=".xlsx"),
                                                br(),
                                                br(),
    ...
    column(12, style = "background-color: white; border-radius: 5px; ",
          ...
          column(width=12,
           ...
           h3("Estimates"),
           tableOutput(ns("wilcoxonTest11")),
           br(),
           h3("Test Results"),
           tableOutput(ns("wilcoxonTest12")),
           br()
    )
    ),
    ...
    column(12, style = "background-color: white; border-radius: 5px; ",
       ...
       br(),
           ...
           br(),
       column(12,
       h4(textOutput(ns("wilcoxonTest1TableTitle"))),
       #br(),
           tableOutput(ns("wilcoxonTest1Table")),
       br(),
       "*Note: The estimate of the magnitude of the group difference given here is not an estimate of the difference in medians, but rather the median of the difference between a sample from the first group and a sample from the second group.",
           br(),
           br(),
       br(),
       br(),
    ...
    column(12, style = " background-color: white; border-radius: 5px; ",
       ...
       you've changed anything along the way. THIS IS ONLY AN EXAMPLE. You must write your own methods and results.",
           br(),
           br(),
           em("After examining the distributions of BMI in the two experimental groups using histograms, parametric tests were ruled out in favor of a Wilcoxon rank-sum test, which was used to test for differences in BMI between the treatment group and the control group. In addition, median group difference was estimated, along with a 95% confidence interval."),
           br(),
           br(),
           h3("Example Results"),
           ...
           br(),
           br(),
           em("Median BMI in the control group was 31.22 kg/m",tags$sup("2"),", with a 95% confidence interval of 
          29.77 kg/m",tags$sup("2")," to 32.28 kg/m",tags$sup("2"),". In the treatment group, the median was
          27.36 kg/m",tags$sup("2"),", with a 95% confidence interval of 
          25.89 kg/m",tags$sup("2")," to 28.84 kg/m",tags$sup("2"),". BMI differed significantly between the 
          two groups (p<0.0001), as the median group difference was 3.88 kg/m",tags$sup("2"),", with a 95% confidence interval of 2.41 kg/m",tags$sup("2")," to 5.03 kg/m",tags$sup("2"),"."),
...
wilcoxon1Server <- function(id) {
    ...
    function(input, output, session) {
      ...
      wilcoxonTest1Table1<-reactive({
       ...
       #medCI2<-MedianCI(b,sides="two.sided")
        ...
        colnames(Results)<-c("Outcome",paste(colnames(matrix1a)," Median (95% CI)",sep=""),paste(colnames(matrix1b)," Median (95% CI)",sep=""),"Median Difference (95% CI)",
                             #"t-Value",
                             "p-Value")
      ...
      output$wilcoxonTest1Table<-renderTable({
        #req(Dat1ab())
        wilcoxonTest1Table1()
      })
      

      output$wilcoxonTest1TableTitle<-renderText({
        paste("Table 1.",outcomeLabel(),"by",predLabel())
      })
      
      wilcoxonTest11a<-reactive({
        ...
        data.frame("Group"=c(colNames1[1],colNames2[1],"Group Difference"),"Estimate"=c(med1,med2,deltaMed),"Lower"=c(lcl1,lcl2,lclDelta),"Upper"=c(ucl1,ucl2,uclDelta))
      })
      
      output$wilcoxonTest11<-renderTable({
        wilcoxonTest11a()
      })
      
      wilcoxonTest12a<-reactive({
      ...
      output$wilcoxonTest12<-renderTable({
        wilcoxonTest12a()
      })
  ...
  output$downloadData<-downloadHandler(filename=function(){
    paste0(tempdir(), "/", "wilcoxonTest1Out.xlsx")
  },
  content=function(file){
    ...
    if(input$dataInput==4){   
      ...
      matrix1b<-input$matrix1b4
        ...
        col1<-matrix(c("Wilcoxon Rank-Sum Test","",
...
#
      ...
      #  colnames(col6)<-b
        ...
        writeData(wb,sheet=sheet,x=wilcoxonTest11a, startCol=6, startRow=4, rowNames=FALSE,colNames=TRUE)
        writeData(wb,sheet=sheet,x="Test Results:", startCol=6, startRow=9, rowNames=FALSE,colNames=FALSE)
        writeData(wb,sheet=sheet,x=wilcoxonTest12a, startCol=6, startRow=10, rowNames=FALSE,colNames=TRUE)
        
        
        table<-wilcoxonTest1Table1()
      ...
      observeEvent(input[["wilcoxonTest1Data"]], {
        xlfile <- input[["wilcoxonTest1Data"]][["datapath"]]
        ext <- tolower(tools::file_ext(xlfile))
        if(ext == "xlsx") {
          sheets <- excel_sheets(xlfile)
          if(length(sheets) == 1L) {
            Dat1ab(read_xlsx(input[["wilcoxonTest1Data"]][["datapath"]], sheet = 1L))
      ...
      observeEvent(input[["sheet"]], {
        if(read_xlsx(input[["wilcoxonTest1Data"]][["datapath"]], sheet = input[["sheet"]])[1,1] != "Wilcoxon Rank-Sum Test"){
          sendSweetAlert(
            session,
            title = "Incorrect File",
            text = "Not a Wilcoxon Rank-Sum Test Analysis File. Please upload an analysis file compatible with the Wilcoxon Rank-Sum Test Tool.",
            type = "error",
            btn_colors="#5f5f62"
          )
        }else{
          Dat1ab(read_xlsx(input[["wilcoxonTest1Data"]][["datapath"]], sheet = input[["sheet"]]))
          dat<-Dat1ab()
